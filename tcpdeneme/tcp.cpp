...
void TCP::insert(const std::string& data) {
    std::unique_lock<std::mutex> lock(mtx);
    TCPMessage message{data, getCurrentDateTime()};
...
void TCP::processQueue() {
    while (true) {
        ...
        while (!messageQueue.empty()) {
            TCPMessage message = messageQueue.front();
            messageQueue.pop();
            lock.unlock();

            // Process the message (for example, send it over a TCP connection)
            std::string formattedMessage = message.dateTime + " Data:" + message.data + "\n";
            std::cout << formattedMessage;

            // Here you would normally send the message over the network using the serverSocket
            int sendResult = send(serverSocket, message.data.c_str(), message.data.size(), 0);
